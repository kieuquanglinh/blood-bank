...
class pagination{
	...
	private $_currentPage; // page đang hiển thị
	...
	public function showPagination(){
		...
		if($this->_totalPage > 1){
			...
			if($this->_currentPage > 1){
				$start 	= "<li class='page-item' ><a class='page-link' href='$actual_link?page=1'>Start</a></li>";
				$prev 	= "<li class='page-item' ><a class='page-link' href='$actual_link?page=".($this->_currentPage-1)."'>«</a></li>";
			}
			$next 	= '';
			$end 	= '';
			...
			for($i = $startPage; $i <= $endPage; $i++){
				if($i == $this->_currentPage) {
					$listPages .= "<li class='page-item active'><a class='page-link' href='#'>".$i.'</a>';
				}else{
		...
		$paginationNav = '<nav aria-label="Page navigation example">' . $paginationHTML . '</nav>';
